// GET /api/fixtures/2026/upcoming — upcoming fixtures, kick-off ascending.

import { NextResponse } from "next/server";
import { createApiErrorResponse } from "@/server/security/api-errors";
import { enforceRateLimit } from "@/server/security/rate-limit";
import { getUpcomingFixtures2026 } from "@/server/fixtures/queries";
...
export async function GET(request: Request) {
  ...
  try {
    const fixtures = await getUpcomingFixtures2026({ limit });
    return NextResponse.json({ count: fixtures.length, fixtures });
  } catch (error) {
    return createApiErrorResponse({
      message: "Upcoming 2026 fixtures are temporarily unavailable.",
      status: 500,
      error,
    });
  }
